...
class SpotifyAPITest extends FunSpec {
	it("Should correctly build a Token obtention URL") {
		assert(SpotifyAPI.buildTokenURI(clientId = "", scope = Seq("")).isFailure)

		val tokenURL = SpotifyAPI.buildTokenURI(clientId = "TEST_ID", scope = Seq("user-read-private", "user-read-email"))
